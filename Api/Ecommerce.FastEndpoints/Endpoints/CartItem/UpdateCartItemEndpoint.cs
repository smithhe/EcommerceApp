...
namespace Ecommerce.FastEndpoints.Endpoints.CartItem
{
	...
	public class UpdateCartItemEndpoint : Endpoint<UpdateCartItemApiRequest, UpdateCartItemResponse>
	{
		...
		public UpdateCartItemEndpoint(ILogger<UpdateCartItemEndpoint> logger, IMediator mediator, ITokenService tokenService)
		{
			this._logger = logger;
		...
		public override async Task HandleAsync(UpdateCartItemApiRequest req, CancellationToken ct)
		{
			...
			catch (Exception e)
			{
				...
				await this.SendAsync(new UpdateCartItemResponse { Success = false, Message = "Unexpected Error Occurred" },
					500, ct);
				return;
			}
