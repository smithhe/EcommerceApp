...
namespace Ecommerce.FastEndpoints.Category
{
	...
	public class CreateCategoryEndpoint : Endpoint<CreateCategoryApiRequest, CreateCategoryResponse>
	{
		...
		public CreateCategoryEndpoint(ILogger<CreateCategoryEndpoint> logger, IMediator mediator, IAuthenticationService authenticationService)
		{
			this._logger = logger;
		...
		public override async Task HandleAsync(CreateCategoryApiRequest req, CancellationToken ct)
		{
			...
			catch (Exception e)
			{
				...
				await SendAsync(new CreateCategoryResponse { Success = false, Message = "Unexpected Error Occurred" },
					500, ct);
				return;
			}
