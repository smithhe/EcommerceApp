...
namespace Ecommerce.FastEndpoints.Category
{
	...
	public class DeleteCategoryEndpoint : Endpoint<DeleteCategoryApiRequest, DeleteCategoryResponse>
	{
		...
		public DeleteCategoryEndpoint(ILogger<DeleteCategoryEndpoint> logger, IMediator mediator, IAuthenticationService authenticationService)
		{
			this._logger = logger;
		...
		public override async Task HandleAsync(DeleteCategoryApiRequest req, CancellationToken ct)
		{
			...
			catch (Exception e)
			{
				...
				await SendAsync(new DeleteCategoryResponse { Success = false, Message = "Unexpected Error Occurred" },
					500, ct);
				return;
			}
