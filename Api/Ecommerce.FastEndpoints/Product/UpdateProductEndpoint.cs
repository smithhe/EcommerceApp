...
namespace Ecommerce.FastEndpoints.Product
{
	...
	public class UpdateProductEndpoint : Endpoint<UpdateProductApiRequest, UpdateProductResponse>
	{
		...
		public UpdateProductEndpoint(ILogger<UpdateProductEndpoint> logger, IMediator mediator, IAuthenticationService authenticationService)
		{
			this._logger = logger;
		...
		public override async Task HandleAsync(UpdateProductApiRequest req, CancellationToken ct)
		{
			...
			catch (Exception e)
			{
				...
				await SendAsync(new UpdateProductResponse { Success = false, Message = "Unexpected Error Occurred" },
					500, ct);
				return;
			}
