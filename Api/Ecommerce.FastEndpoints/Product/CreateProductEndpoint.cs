...
namespace Ecommerce.FastEndpoints.Product
{
	...
	public class CreateProductEndpoint : Endpoint<CreateProductApiRequest, CreateProductResponse>
	{
		...
		public CreateProductEndpoint(ILogger<CreateProductEndpoint> logger, IMediator mediator, IAuthenticationService authenticationService)
		{
			this._logger = logger;
		...
		public override async Task HandleAsync(CreateProductApiRequest req, CancellationToken ct)
		{
			...
			catch (Exception e)
			{
				...
				await SendAsync(new CreateProductResponse { Success = false, Message = "Unexpected Error Occurred" },
					500, ct);
				return;
			}
