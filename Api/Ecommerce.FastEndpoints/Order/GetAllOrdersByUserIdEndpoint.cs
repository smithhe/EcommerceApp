...
namespace Ecommerce.FastEndpoints.Order
{
	...
	public class GetAllOrdersByUserIdEndpoint : Endpoint<GetAllOrdersByUserIdApiRequest, GetAllOrdersByUserIdResponse>
	{
		...
		public GetAllOrdersByUserIdEndpoint(ILogger<GetAllOrdersByUserIdEndpoint> logger, IMediator mediator, IAuthenticationService authenticationService)
		{
			this._logger = logger;
		...
		public override async Task HandleAsync(GetAllOrdersByUserIdApiRequest req, CancellationToken ct)
		{
			...
			catch (Exception e)
			{
				...
				await SendAsync(new GetAllOrdersByUserIdResponse { Success = false, Message = "Unexpected Error Occurred" },
					500, ct);
				return;
			}
